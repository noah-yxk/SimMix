...
class smixup():
    ...
    def __init__(self, data_root_path, dataset, SupCon_conf):
        self._get_dataset(data_root_path, dataset)
        self.SupCon_conf = SupCon_conf
        # self._get_SupCon(self.SupCon_conf['nlayers'], self.SupCon_conf['nhidden'], self.SupCon_conf['bs'], self.SupCon_conf['lr'], self.SupCon_conf['epochs'])
      
    def _get_SupCon(self, SupCon_nlayers, SupCon_hidden, SupCon_bs, SupCon_lr, SupCon_epochs):  
        conf = {}
        conf_dis_param = {}
        conf_dis_param['num_layers'] = SupCon_nlayers
        conf_dis_param['hidden'] = SupCon_hidden
        conf_dis_param['model_type'] = 'SupCon'
        conf_dis_param['pool_type'] = 'sum'
        conf_dis_param['fuse_type'] = 'abs_diff'
        conf['dis_param'] = conf_dis_param

        conf['batch_size'] = SupCon_bs
        conf['start_lr'] = SupCon_lr
        conf['factor'] = 0.5
        conf['patience'] = 5
        conf['min_lr'] = 0.0000001
        conf['pre_train_path'] = None
        conf['max_num_epochs'] = SupCon_epochs

        conf['dis_param']['in_dim'] = self.dataset[0].x.shape[1]
        
        self.SupCon = GraphMatching(**conf['dis_param'])
    ...
    def train_test(self, batch_size, cls_model, cls_nlayers, cls_hidden, cls_dropout, cls_lr, cls_epochs, alpha1, alpha2, ckpt_path, sim_method = 'cos'):
        r"""
        This method first train a SupCon and then use the SupCon to perform S-Mixup. 
        ...
        """
        print("ifplus")
        # print(self.SupCon_conf)
        print("batch_size: {}, cls_model: {}, cls_nlayers: {}, cls_hidden: {}, cls_dropout: {}, cls_lr: {}, cls_epochs: {}, alpha1: {}, ckpt_path: {}, sim_method: {}".format(batch_size, cls_model, cls_nlayers, cls_hidden, cls_dropout, cls_lr, cls_epochs, alpha1, ckpt_path, sim_method))
        ...
        for i, (train_idx, test_idx) in enumerate(kf.split(list(range(len(self.dataset))))):
            ...
            for epoch in range(1, cls_epochs + 1):
                ...
                for step, batch in enumerate(train_loader):
                    batch = self.ifplus(batch, model = model, alpha = alpha1)
        ...
        for epoch in range(1, self.SupCon_conf['epochs'] + 1):
            ...
            for step, batch in enumerate(train_loader):
                ...
                if epoch % 100 == 1:
                    ...
                    all_labels.append(batch.y.long().cpu().numpy())
            ...
            if epoch % 100 == 1:
                ...
                plt.title(f'Epoch {epoch} Embeddings Visualization')
                
                output_dir = "visualizations_IMDBM"
                os.makedirs(output_dir, exist_ok=True)
                plt.savefig(f"{output_dir}/epoch_{epoch}_embeddings.png")
                plt.close()
